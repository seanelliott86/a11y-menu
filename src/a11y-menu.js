    ...
    if(event.type === "keydown"){
        ...
        if(!inSubMenu && !this.allowKey([38,40,37,39,32,13,27,9,16,36,35], key)){
            ...
            if(subMenuOpen){
                ...
                if(this.allowKey([40,38], key)) return false;
...
A11yMenu.prototype.travelMain = function(event, key){
    ...
    if(this.allowKey(allowedKeys, key)) return false;
...
A11yMenu.prototype.openEvent = function(event, key){
    ...
    if(this.allowKey(allowedKeys, key) || event.target.classList.contains(this.option.activeClass)) return false; 
...
A11yMenu.prototype.travelSub = function(event, key){
    if(this.allowKey([37,38,39,40,9,35,36], key)) return false;
    ...
    var upDown = function(){
        if(_this.allowKey([38,40], key)) return false;
    ...
    var leftRight = function(){
        if(_this.allowKey([37,39], key) || !_this.option.horizontalMenu) return false;
    ...
    var tab = function(){
        if (key !== 9 || option.tapStop) return false;
...
A11yMenu.prototype.homeEnd = function(event, links, key){
    if(this.allowKey([35,36], key)) return false;
